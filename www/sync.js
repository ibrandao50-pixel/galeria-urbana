...
let likeCounts={},likedWorks=new Set();const voterId=localStorage.getItem('rua_voter_id')||crypto.randomUUID();localStorage.setItem('rua_voter_id',voterId);
...
async function loadLikes(){const{data,error}=await cloud.from('obra_curtidas').select('obra_id,voter_id');if(error){console.warn('Falha ao atualizar curtidas',error.message);return}likeCounts={};likedWorks=new Set();for(const like of data){const id=String(like.obra_id);likeCounts[id]=(likeCounts[id]||0)+1;if(like.voter_id===voterId)likedWorks.add(id)}window.bindLikeButtons?.()}
window.bindLikeButtons=()=>document.querySelectorAll('.like-btn').forEach(button=>{const id=String(button.dataset.like);button.classList.toggle('liked',likedWorks.has(id));button.innerHTML=`${likedWorks.has(id)?'♥':'♡'} <span>${window.getLikeCount(id)}</span>${button.classList.contains('large')?' curtidas':''}`;button.onclick=async event=>{event.preventDefault();event.stopPropagation();button.disabled=true;const{data,error}=await cloud.rpc('alternar_curtida',{p_obra_id:id,p_voter_id:voterId});button.disabled=false;if(error)return syncNotice('Não foi possível registrar a curtida.',true);const result=Array.isArray(data)?data[0]:data;likeCounts[id]=Number(result.like_count);result.liked?likedWorks.add(id):likedWorks.delete(id);document.querySelectorAll(`[data-like="${CSS.escape(id)}"]`).forEach(el=>{el.classList.toggle('liked',result.liked);el.classList.add('pulse');el.innerHTML=`${result.liked?'♥':'♡'} <span>${result.like_count}</span>${el.classList.contains('large')?' curtidas':''}`;setTimeout(()=>el.classList.remove('pulse'),350)})}});
...
cloud.auth.signOut({scope:'local'}).finally(()=>{admin=false;localStorage.removeItem('rua_admin');render()});
function storagePathFromUrl(url){if(!url||!url.includes('/storage/v1/object/public/imagens-obras/'))return null;try{return decodeURIComponent(new URL(url).pathname.split('/storage/v1/object/public/imagens-obras/')[1]||'')}catch{return null}}
document.querySelector('#deleteBtn').onclick=async()=>{const id=document.querySelector('#artId').value;if(!id)return;const art=arts.find(item=>String(item.id)===String(id));if(!confirm(`Excluir definitivamente “${art?.title||'esta obra'}”?\n\nA obra e sua fotografia serão removidas para todos os visitantes.`))return;const button=document.querySelector('#deleteBtn');button.disabled=true;button.textContent='Excluindo…';try{const imagePath=storagePathFromUrl(art?.image);if(imagePath){const{error:storageError}=await cloud.storage.from('imagens-obras').remove([imagePath]);if(storageError)throw storageError}const{error}=await cloud.from('obras').delete().eq('id',id);if(error)throw error;arts=arts.filter(item=>String(item.id)!==String(id));originalSetItem.call(localStorage,'rua_arts',JSON.stringify(arts));document.querySelector('#editModal').close();render();if(map)initMap();syncNotice('Obra e fotografia excluídas para todos.')}catch(error){syncNotice(`Não foi possível excluir: ${error.message}`,true)}finally{button.disabled=false;button.textContent='Excluir obra'}};
...
if(window.RUA_FEATURES?.likes){
  cloud.channel('curtidas-publicas').on('postgres_changes',{event:'*',schema:'public',table:'obra_curtidas'},()=>loadLikes()).subscribe();
  loadLikes();
  ...
  setInterval(()=>{if(!document.hidden)loadLikes()},10000);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)loadLikes()});
